// "forked" version of https://github.com/pubsubio/pubsub-hub/blob/master/node_modules/querify.js
...
var inner = {
	$not: function(key, val) {
		...
		return function(doc) {
			return getPathValue(key, doc) !== val;
		};
	},
	$exists: function(key, val) {
		return function(doc) {
			var targetValue = getPathValue(key, doc);
			return (targetValue !== null) === val;
		};
	},
	$nil: function(key, val) {
		return function(doc) {
			var targetValue = getPathValue(key, doc);
			return (targetValue === undefined || targetValue === null) === val;
		};
	},
	...
	$like: function(key, vals) {
		return reduceArray('and', vals, function(val) {
			return function(doc) {
				var targetValue = getPathValue(key, doc);
				return typeof targetValue === 'string' && targetValue.toLowerCase().indexOf(val) > -1;				
			};
		});
	},
	$regex: function(key, regex) { // normalized syntax for regex queries
		...
		return function(doc) {
			var targetValue = getPathValue(key, doc);
			return targetValue !== undefined && targetValue !== null && regex.test(targetValue);
		};
	},
	...
	$mod: function(key, val) {
		return function(doc) {
			return getPathValue(key, doc) % val[0] === val[1];
		};
	},
	/* removed because it was dependent of another nodejs module.
	$distance: function(key, val) {
		...
		var distance = Distance(radius);
		
		return function(doc) {
			var point = doc[key];
			
			return Distance.within(center,point,distance);
		};
	},*/
	$datetime: function(key, val) {
		var template = /^(?:(mon|tue|wed|thu|fri|sat|sun)\w*)?,?\s*(\d{1,2}(?:[^:\d]|$))?\s*([a-z]+)?\s*(\d{4})?\s*([\d|-]{1,2}:[\d|-]{1,2}:[\d|-]{1,2})?\s*([+-]\d*\.?\d+)?/i;		
...
var compile = function(query) {
	...
	list = push(list, function(doc) {
		for (var i in subset) {
			if (subset[i] !== getPathValue(i, doc)) {
				return false;
			}
		}
		return true;
...
// get deeply nested values, by a dot seperated string.
// ex: 
// var obj = {contact: {mail: 'x@x.x'}}
// var mail = getPathValue('contact.mail', obj);
function getPathValue(path, target) {
	var segments = path.split(".");
	var pointer = target;
	while (segments.length > 0) {
		var index = segments.shift();
		if (typeof pointer[index] == 'undefined') {
			return null;
		}
...
console.log(compile({point: {$distance:{center:berlin,radius:'857000 m'}}})({point:oslo}));
